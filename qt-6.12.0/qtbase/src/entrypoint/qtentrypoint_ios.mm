...
/*
    We replace the application delegate with our QIOSApplicationDelegate
    later on, but we need a placeholder delegate that implements dynamic
    scene management, otherwise iOS will mistakenly complain that we
    haven't adopted the UIScene lifecycle yet.
*/
@interface QIOSDynamicSceneDelegate : UIResponder <UIApplicationDelegate>
@end

@implementation QIOSDynamicSceneDelegate
- (UISceneConfiguration *)application:(UIApplication *)application
                          configurationForConnectingSceneSession:(UISceneSession *)session
                          options:(UISceneConnectionOptions *)options
{
    // If we're connecting a session that has its configuration in
    // the Info.plist we just want to pass on the static config.
...
extern "C" int qt_main_wrapper(int argc, char *argv[])
{
    @autoreleasepool {
        ...
        return UIApplicationMain(argc, argv, nil,
            NSStringFromClass(QIOSDynamicSceneDelegate.class));
#endif
    }
}
...
void registerApplicationLifecycleCallbacks()
{
    [[NSNotificationCenter defaultCenter]
        addObserver:UIApplication.class
        selector:@selector(qt_applicationDidFinishLaunching:)
        name:UIApplicationDidFinishLaunchingNotification
        object:nil];

    [[NSNotificationCenter defaultCenter]
        addObserver:UIApplication.class
        selector:@selector(qt_applicationWillTerminate)
        name:UIApplicationWillTerminateNotification
        object:nil];
}
Q_CONSTRUCTOR_FUNCTION(registerApplicationLifecycleCallbacks)

@implementation UIApplication (QtRunLoopIntegration)
...
+ (void)qt_applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    if (![self qt_rootLevelRunLoopIntegration]) {
...
+ (void)qt_applicationWillTerminate
{
    ...
    case kJumpPointSetSuccessfully:
        ...
        qApp->exit([self qt_applicationWillTerminateExitCode]);
        ...
        [self qt_eventDispatcherInterruptEventLoopExec];
        break;
...
{
    return applicationAboutToTerminate;
}
