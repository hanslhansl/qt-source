...
This repository contains the source code for Qt, currently version 6.12.0.
It serves as archive for other projects, mainly for licensing purposes but also for a simplified build process.

The source is based on the Qt source distribution obtained from the official Qt installer.

## Modifications

The following modifications were made to the Qt source used to build the distributed binaries:

- Added `#include <vector>` to /qt/qttools/src/linguist/shared
...
See the `qt/LICENSES/` directory for the applicable Qt License texts and licensing information.

## Building

`build_qt.py` is provided to reproduce the Qt binaries.

The script builds the Qt source contained in `qt/`.
